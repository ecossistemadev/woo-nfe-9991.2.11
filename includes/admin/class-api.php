	...
	class NFe_Woo {
		...
		public function order_info( $order_id ) {
			...
			$data = array(
				...
				'description'        => "Nota referente ao pedido #".$order_id."\n\n".$servicesDescription,
		...
		protected function check_customer_info( $field = '', $order ) {
			...
			if ( in_array( $field, array( 'number', 'name', 'type' ), true ) ) {
			    $document_type = get_post_meta( $order, '_billing_document_type', true);
				$customer = get_post_meta( $order, '_billing_first_name', true ) . ' ' . get_post_meta( $order, '_billing_last_name', true );
				$cpf = get_post_meta( $order, '_billing_document', true );
				$cnpj = get_post_meta( $order, '_billing_document', true );
